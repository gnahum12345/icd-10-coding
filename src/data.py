...
class ICD10HierarchyLoader:
    ...
    def _parse_diag(self, diag_elem: ET.Element, parent: ICD10Node) -> Optional[ICD10Node]:
...
class ICD10Dataset:
    ...
    def get_metrics(self, predictions: List[List[str]]) -> Dict[str, float]:
        ...
        if len(predictions) != len(self):
            raise ValueError(f"Predictions length ({len(predictions)}) doesn't match dataset length ({len(self)})")
        ...
        micro_precision = true_positives / len(all_pred_codes) if len(all_pred_codes) > 0 else 0.0
        micro_recall = true_positives / len(all_gt_codes) if len(all_gt_codes) > 0 else 0.0
        ...
        macro_precision = sum(sample_precisions) / len(sample_precisions) if sample_precisions else 0.0
        macro_recall = sum(sample_recalls) / len(sample_recalls) if sample_recalls else 0.0
        ...
        if self.hierarchy_loader is not None:
            ...
            hierarchical_accuracy = hierarchical_matches / len(self) if len(self) > 0 else 0.0
