...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ICD10 Code Extraction Evaluation"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--uncertainty",
        action="store_true",
        help="Run uncertainty quantification"
    )
    parser.add_argument(
        "--n-mc-samples",
        type=int,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_one, i, config, args.uncertainty, hierarchy_loader, dataset[i]): i
    ...
    metrics = evaluator.compute_metrics(
        predictions,
        ground_truth,
        confidences if args.uncertainty else None
    )
    wrong_cases = evaluator.collect_wrong_cases(
        predictions,
        ground_truth,
        [t for th in traces for t in th]
    )
